...
import com.shs.framework.utils.HandlerUtils;

/**
 * 替代 StatViewServlet
 */
public class DruidStatViewHandler extends AbstractHandler {
	
	private IDruidStatViewAuth auth;
	private String visitPath = "/druid";
	private StatViewServlet servlet = new DruidStatViewServlet();
	...
	public void handle(String target, HttpServletRequest request, HttpServletResponse response, boolean[] isHandled) {
		if (target.startsWith(visitPath)) {
			...
			if (target.equals(visitPath) && !target.endsWith("/index.html")) {
				HandlerUtils.redirect(target += "/index.html", request, response, isHandled);
				return ;
			}
